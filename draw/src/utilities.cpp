...
Int_t msgBoxQuestion(const char* title, const char* text){
  ...
  new TGMsgBox(gClient->GetRoot(), NULL, title, text,
               EMsgBoxIcon::kMBIconQuestion,
               EMsgBoxButton::kMBYes | EMsgBoxButton::kMBNo, &ret);
  return ret;
}
...
TPad* addOverlayPad(TCanvas* canvas, Int_t padNumberToOverlay /* 1st = 1, 2nd = 2, ...*/, Int_t currentNPads /* current number of pads */){
  ...
  TPad* overlayPad = new TPad(overlayPadName.Data(), "", bottomPad->GetXlowNDC(), bottomPad->GetYlowNDC(), bottomPad->GetXlowNDC()+bottomPad->GetWNDC(), bottomPad->GetYlowNDC()+bottomPad->GetHNDC());
...
TList* getFileNamesList(const char* text, Bool_t isMultiple){
  ...
  if (isMultiple) fi.SetMultipleSelection(kTRUE);
...
Int_t findFirstNonZeroBin(std::vector<TH1*> histList){
  int minFirstNonZeroBin = INT_MAX;
  for (TH1* hist : histList) {
    Int_t firstNonZeroBin = findFirstNonZeroBin(hist);
    if (firstNonZeroBin < minFirstNonZeroBin) minFirstNonZeroBin = firstNonZeroBin;
  }
...
Int_t findLastNonZeroBin(std::vector<TH1*> histList){
  ...
  for(TH1* hist : histList){
    ...
    if (lastNonZeroBin > maxLastNonZeroBin) maxLastNonZeroBin = lastNonZeroBin;
  }
...
TH2* makeHistQueue(std::vector<TH1*> histList, const char* name, Double_t crystalLength){
  ...
  TH2* histQueue = new TH2D(name, "",
                            firstHist->GetXaxis()->GetNbins(),
                            firstHist->GetXaxis()->GetBinLowEdge(1),
                            firstHist->GetXaxis()->GetBinUpEdge(firstHist->GetXaxis()->GetNbins()),
                            nUnitVolumes, 0, crystalLength);
  Int_t y = 1;
  for (TH1* hist : histList){
...
Double_t getTGraphValue(Double_t x, TGraph* graph){
  ...
  if (x < graph->GetPointX(0)) return 0;
  if (x > graph->GetPointX(graph->GetN()-1)) return 0;
...
TH1* convertOpToPe(TH1* histOp, const char* detector){
  ...
  for (int bin = 1; bin <= histOp->GetXaxis()->GetNbins(); bin++){
    ...
    for (int p = 1; p <= pe; p++) histPe->Fill(histOp->GetBinCenter(bin));
  }
...
Double_t getGraphMax(TGraph* graph){
  ...
  for (int i = 1; i < graph->GetN(); i++){
    if (graph->GetPointY(i) > max) max = graph->GetPointY(i);
  }
  return max;
}
...
Int_t exportTreeToGnuplot(TTree* tree, TString filename, const char* delimeter, Bool_t append){
  ...
  for (int i=0; i < (int)colNames.size(); i++){
    ...
    if (i != (int)colNames.size()-1) myfile << delimeter;
  }
  myfile << std::endl;
  ...
  for (int i = 0; i < tree->GetEntries(); i++) {
     ...
     for (int j=0; j < nBranches; j++){
       ...
       if (j != nBranches-1) myfile << delimeter;
     }
     myfile << std::endl;
  }
  return 0;
...
void saveCanvasToDisk(TCanvas* canvas, const char* name, Bool_t savePNG, Bool_t saveC, Bool_t saveROOT){
  if (savePNG){
    TString pngName = name; pngName += ".png";
  ...
  if (saveROOT){
    TString rootName = name; rootName += ".root";
...
void saveGraphToDisk(TGraph* graph, const char* name, Bool_t saveGNUPLOT, Bool_t saveROOT){
  ...
  if (saveGNUPLOT){
    TString gnuplotName = name; gnuplotName += ".dat";
...
Double_t InvCrystalBallFunctionObject::operator() (double *_x, double *par) {
  LongDouble_t x = _x[0];
  LongDouble_t a = par[0];
  LongDouble_t n = par[1];
  LongDouble_t mean = par[2];
...
TF1* fitHistWithCrystalBall(TH1* hist, Bool_t inverted){
  TF1* fBall = new TF1();
  if (!inverted){
    CrystalBallFunctionObject* fFunctionObject = new CrystalBallFunctionObject();
    fBall = new TF1("fBall", *fFunctionObject, hist->GetXaxis()->GetXmin(), hist->GetXaxis()->GetXmax(), 5);    // create TF1 class.
  }
  else {
    InvCrystalBallFunctionObject* fFunctionObject = new InvCrystalBallFunctionObject();
    fBall = new TF1("invFBall", *fFunctionObject, hist->GetXaxis()->GetXmin(), hist->GetXaxis()->GetXmax(), 5);    // create TF1 class.
  }
  fBall->SetNpx(500);
  ...
  fBall->SetParameter(2, hist->GetBinCenter(hist->GetMaximumBin()));
  // fBall->SetParLimits(2, hist->GetXaxis()->GetXmin(), hist->GetXaxis()->GetXmax());

  fBall->SetParName(3, "sigma");
  fBall->SetParameter(2, hist->GetRMS());
  // fBall->SetParLimits(3, 0, hist->GetRMS()*2);

  fBall->SetParName(4, "norm");
  fBall->SetParameter(4, hist->Integral("width"));
  // fBall->SetParLimits(4, hist->Integral("width")/2, hist->hist("width")*2);
