   ...
   fWrapMaterName("")
{
  ...
  // fPMT_cathode_distance = 0*mm; // 1.5*mm
  // fPMT_cathode_thickness = 10*mm;
  // fPMT_length = 88*mm;
  ...
  fUsePrimitiveScorer = true;

  // Write world escape energy and locations
  fUseWorldEscapeAction = true;

  // Default value for using Scoring Mesh
  fUseGlobalScoringMesh = true;
  fUseUnitVolumeScoringMesh = false;
  ...
  // Instantiate local class members for materials
  // DefineMaterials();
...
void DetectorConstruction::DefineMaterials()
{
  ...
  fOuterDetectorMater = fOuterCrystalType.length() == 0 ?
                        NULL :
  ...
  // Optical grease
  fGreaseMater = (fGreaseType == GreaseType::PDMS) ?
                 Materials::getInstance()->getMaterial("PDMS") :
								 Materials::getInstance()->getMaterial("BC630");
...
//  G4double fWorld_Z = (fCrystal_Z + detector->getLength() + 20*cm)*2;
  ...
  G4Box* sWorld = new G4Box("world_solid", 0.5*fWorld_max, 0.5*fWorld_max, 0.5*fWorld_max); //its size
  G4LogicalVolume* lWorld = new G4LogicalVolume(sWorld,       // its solid
                                                fWorldMater,  // its material
                                                "world_logical"); //its name
...
//                                               fCheckOverlaps);    // checking overlaps
  ...
  if (fWrapMater){
    G4Box* wrapASolid = new G4Box("wrapA_solid", 0.5*(fCrystal_X + 2*fWrapThickness), 0.5*(fCrystal_Y + 2*fWrapThickness), 0.5*(fCrystal_Z + fWrapThickness));
    G4Box* wrapBSolid = new G4Box("wrapB_solid", 0.5*fCrystal_X, 0.5*fCrystal_Y, 0.5*(fCrystal_Z + fWrapThickness));
    ...
    G4VPhysicalVolume* wrapPV = new G4PVPlacement(0,
    ...
    if (isHybrid()){
      G4VPhysicalVolume* wrapOutPV = new G4PVPlacement(0,
                                       G4ThreeVector(0, 0, wrap_pos_Z),
                                       fLogicWrap,
...
//  }
  ...
  if (AbsPMT* pmt = dynamic_cast<AbsPMT*>(detector)){
    ...
    G4Tubs* pmtWindowSolid = new G4Tubs("pmtWindow_solid", 0, pmt->getCathodeDiameter()/2, pmt->getWindowThickness()/2, 0, twopi);
    ...
    G4Tubs* pmtCathodeSolid = new G4Tubs("pmtCathode_solid", 0, pmt->getCathodeDiameter()/2, pmt->getCathodeThickness()/2, 0, twopi);
    ...
    G4double pmtVacuumLength = pmt->getLength() - pmt->getWindowThickness() - pmt->getCathodeThickness() - pmtWallThickness;
    G4Tubs* pmtVacuumSolid = new G4Tubs("pmtVacuum_solid", 0, pmt->getCathodeDiameter()/2, pmtVacuumLength/2, 0, twopi);
    fLogicPMTVacuum = new G4LogicalVolume(pmtVacuumSolid, fPMTVacuumMater, "pmtVacuum_logical");
    G4double pmtVacuumPosZ = - pmt->getLength()/2 + pmtVacuumLength/2 + pmt->getWindowThickness() + pmt->getCathodeThickness();
    new G4PVPlacement(0,
    ...
    G4Tubs* sPMTcoverInner = new G4Tubs("pmtCoverIn_solid", 0, pmt->getDiameter()/2, pmt->getLength()/2 + 1, 0, twopi); // + 1 just in case for better subtraction

    G4RotationMatrix* coverRot = new G4RotationMatrix();
    G4ThreeVector coverTrans(0, 0, 0);
    G4SubtractionSolid* sPMTcover = new G4SubtractionSolid("pmtCover_solid", sPMTcoverOuter, sPMTcoverInner, coverRot, coverTrans);
  ...
  // If MPPC type detector
    ...
    G4Box* mppcReflectorOuter = new G4Box("mppcReflectorOuterSolid", 0.5*(single_X), 0.5*(single_Y), 0.5*mppc->getLength());
    G4Box* mppcReflectorInner = new G4Box("mppcReflectorInnerSolid", 0.5*mppc->getSize(), 0.5*mppc->getSize(), 0.5*mppc->getLength());
    G4RotationMatrix* reflectorRot = new G4RotationMatrix();
    G4ThreeVector reflectorTrans(0, 0, 0);
    G4SubtractionSolid* mppcReflectorSolid = new G4SubtractionSolid("mppcReflector_solid", mppcReflectorOuter, mppcReflectorInner, reflectorRot, reflectorTrans);

    fLogicMPPCShield = new G4LogicalVolume(mppcReflectorSolid, fMPPCShieldMater, "mppcReflector_logical");
    G4double mppcReflectorPosZ = 0.5*single_Z - fGreaseThickness - detector->getLength()/2;
    new G4PVPlacement(0,
                      G4ThreeVector(0, 0, mppcReflectorPosZ),
                      fLogicMPPCShield,
                      "mppcCase_physical",
                      singleLV,
                      false,
    ...
    G4Box* mppcWindowSolid = new G4Box("MPPC_Win_sol", 0.5*mppc->getSize(), 0.5*mppc->getSize(), 0.5*mppc->getWindowThickness());
    fLogicMPPCWindow = new G4LogicalVolume(mppcWindowSolid, mppc->getWindowMaterial(), "mppcWindowLog");
    G4double mppcWindowPosZ = 0.5*single_Z - detector->getLength() + detector->getWindowThickness()/2;
    new G4PVPlacement(0,
                      G4ThreeVector(0, 0, mppcWindowPosZ),
                      fLogicMPPCWindow,
                      "mppcWindow_physical",
                      singleLV,
                      false,
    ...
    G4Box* mppcSolid = new G4Box("mppc_solid", 0.5*mppc->getSize(), 0.5*mppc->getSize(), 0.5*mppc->getCathodeThickness());
...
//  if (singleOutLV!=NULL) singleOutLV->SetVisAttributes(new G4VisAttributes(G4Colour::Red()));
  ...
  if (singleOutLV!=NULL) singleOutLV->SetVisAttributes(G4VisAttributes::GetInvisible());


  fLogicCrystal->SetVisAttributes(new G4VisAttributes(G4Colour::Cyan()));

  if (fLogicWrap) fLogicWrap->SetVisAttributes(new G4VisAttributes(false));
  ...
  if (fLogicPMTshield) fLogicPMTshield->SetVisAttributes(new G4VisAttributes(G4Colour::Gray()));
  ...
  if (fLogicMPPCShield) fLogicMPPCShield->SetVisAttributes(new G4VisAttributes(G4Colour::White()));
}
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  if (fUsePrimitiveScorer){
    G4MultiFunctionalDetector* detectorCrystal = new G4MultiFunctionalDetector(MultiFunctionalDetectorNames::CRYSTAL_VOLUME);
...
void DetectorConstruction::SetSaveEnergyDeposition(G4bool value){
...
G4bool DetectorConstruction::GetSaveEnergyDeposition(){
