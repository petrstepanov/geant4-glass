...
HistoManager::HistoManager()
  : fFileNamePattern("file_x_%dmm_y_%dmm_z_%dmm_%.1fGeV.root"),
    ...
    fNtupleCrystals(0),
    fEdep {0},                  // PS: use the initializer list for array members (C++11)
    fEdepTotal(0),
    // Energy depositino in unit volumes tree
    fNtupleUnitVolumes(0),
    fUnitVolumeEnergies(0),
    ...
    fPhotonsProduced(0),
    fScintPhotonsProduced(0),
    fCherePhotonsProduced(0),
    fPhotonsDetected(0),
    fScintPhotonsDetected(0),
    fCherePhotonsDetected(0),


    fPhotoElectrons(0),
    fScintPhotoElectrons(0),
    fCherePhotoElectrons(0),
    ...
    fPhotonsProducedHists(0),
    fScintPhotonsProducedHists(0),
    fCherePhotonsProducedHists(0),

    fPhotonsProducedHist(0),
    fScintPhotonsProducedHist(0),
    fCherePhotonsProducedHist(0),

    fPhotonsDetectedHists(0),
    fScintPhotonsDetectedHists(0),
    fCherePhotonsDetectedHists(0),

    fPhotonsDetectedHist(0),
    fScintPhotonsDetectedHist(0),
    fCherePhotonsDetectedHist(0),
...
void HistoManager::Book()
{
  ...
  fScintPhotonsProducedHist = new TH1I("prod_op_scint", "Scintillation OP produced in all crystals", wlMax-wlMin, wlMin, wlMax);
  fCherePhotonsProducedHist = new TH1I("prod_op_chere", "Cherenkov OP produced in all crystals", wlMax-wlMin, wlMin, wlMax);
  ...
  fScintPhotonsDetectedHist = new TH1I("det_op_scint", "Scintillation OP detected in all photo-cathodes", wlMax-wlMin, wlMin, wlMax);
  fCherePhotonsDetectedHist = new TH1I("det_op_chere", "Cherenkov OP detected in all photo-cathodes", wlMax-wlMin, wlMin, wlMax);
  ...
  for (G4int i = 0; i < G4Utils::getNCrystals(); i++){
    // Photon spectra (with wavelength) produced in each crystal
    fPhotonsProducedHists[i] = new TH1I(TString::Format("prod_op_%d", i), TString::Format("OP produced in crystal #%d", i), wlMax-wlMin, wlMin, wlMax);
    fScintPhotonsProducedHists[i] = new TH1I(TString::Format("prod_op_scint_%d", i), TString::Format("Scintillation OP produced in crystal #%d", i), wlMax-wlMin, wlMin, wlMax);
    fCherePhotonsProducedHists[i] = new TH1I(TString::Format("prod_op_chere_%d", i), TString::Format("Cherenkov OP produced in crystal #%d", i), wlMax-wlMin, wlMin, wlMax);

    // Photon spectra (with wavelength) detected in each crystal
    fPhotonsDetectedHists[i] = new TH1I(TString::Format("det_op_%d", i), TString::Format("OP detected from crystal #%d", i), wlMax-wlMin, wlMin, wlMax);
    fScintPhotonsDetectedHists[i] = new TH1I(TString::Format("det_op_scint_%d", i), TString::Format("Scintillation OP detected from crystal #%d", i), wlMax-wlMin, wlMin, wlMax);
    fCherePhotonsDetectedHists[i] = new TH1I(TString::Format("det_op_chere_%d", i), TString::Format("Cherenkov OP detected from crystal #%d", i), wlMax-wlMin, wlMin, wlMax);
  }
...
Bool_t HistoManager::producedHistIsFull(){
  if (fPhotonsProducedHist->GetBinContent(fPhotonsProducedHist->GetMaximumBin()) < INT_MAX - 10) return false;
  return true;
}
...
void HistoManager::Save()
{
  ...
  std::cout << "Incident particle energy: " << energyIncidentParticleEVU.value << " " << energyIncidentParticleEVU.unit << std::endl;
  RooConstVar* gpsEnergy = new RooConstVar("gpsParticleEnergy", "Incident particle energy, MeV", G4Utils::getGPSMonoEnergy());
  gpsEnergy->Write();
  ...
  std::cout << "Deposited in crystals - from scorer: " << energyTotalCrystalsScorerEVU.value << " " <<  energyTotalCrystalsScorerEVU.unit << " (" << fEdepTotal/energyTotalGPS*100 << " %)"  << std::endl;
  ...
  if (G4Utils::getDetectorConstruction()->GetUseGlobalScoringMesh()){
    ...
    std::cout << "Deposited in crystals - from global mesh: " << energyTotalCrystalsMeshEVU.value << " " << energyTotalCrystalsMeshEVU.unit << " (" << energyTotalCrystalsMesh/energyTotalGPS*100 << " %)"  << std::endl;
    RooConstVar* energyTotalCrystalsMeshVar = new RooConstVar("energyTotalCrystalsMesh", "Total energy in crystals from mesh, MeV", energyTotalCrystalsMesh);
    energyTotalCrystalsMeshVar->Write();

    // Write total energy in PMTs from mesh
    G4double energyTotalPMTsMesh = G4Utils::getTotalQuantityFromMesh(MultiFunctionalDetectorNames::PMTS_MESH, HitsCollectionNames::ENE_DEP_PMTS_MESH);
    EnergyValueUnit energyTotalPMTsMeshEVU = G4Utils::formatEnergy(energyTotalPMTsMesh);
    std::cout << "Deposited in detectors - from global mesh: " << energyTotalPMTsMeshEVU.value << " " << energyTotalPMTsMeshEVU.unit << " (" << energyTotalPMTsMesh/energyTotalGPS*100 << " %)" << std::endl;
    RooConstVar* energyTotalPMTsMeshVar = new RooConstVar("energyTotalPMTsMesh", "Total energy in PMTs from mesh, MeV", energyTotalCrystalsMesh);
...
//  }
  ...
  std::cout << "Escaped the world: " << energyEscapedWorldEVU.value << " " << energyEscapedWorldEVU.unit << " (" << fTotalOutWorldEnergy/energyTotalGPS*100 << " %)" << std::endl;
  RooConstVar* energyTotalOutWorldVar = new RooConstVar("energyTotalOutWorld", "Total energy escaped the world, MeV", fTotalOutWorldEnergy);
