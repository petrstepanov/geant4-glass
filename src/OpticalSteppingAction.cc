...
void OpticalSteppingAction::UserSteppingAction(const G4Step* step) {
  ...
  Int_t firstTOFNtupleId = analysisManager->GetFirstNtupleId();
  Int_t eventNumber = G4RunManager::GetRunManager()->GetCurrentEvent()->GetEventID();
  Int_t currentNtupleId = firstTOFNtupleId + eventNumber + 1;
